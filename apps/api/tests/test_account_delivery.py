...
def test_resend_cooldown_preserves_original_token(client, db_factory):
    ...
    with db_factory() as db:
        tokens = list(db.find(PasswordResetToken))
        assert len(tokens) == 1 and tokens[0].used_at is None
        assert (
            tokens[0].token_hash == hashlib.sha256(first.json()["reset_token"].encode()).hexdigest()
        )
...
def test_used_email_is_not_delivered(client, monkeypatch, db_factory):
    registered = register(client)
    client.post(
        "/api/v1/auth/email-verification/verify", json={"token": registered["verification_token"]}
    )
    send = MagicMock()
    monkeypatch.setattr("app.mail_worker.send_account_email", send)
    with db_factory() as db:
        assert deliver_one(db)
        assert db.find_one(AccountEmail).status == "obsolete"
...
def test_expired_email_is_not_delivered(client, monkeypatch, db_factory):
    ...
    monkeypatch.setattr("app.mail_worker.send_account_email", send)
    with db_factory() as db:
        item = db.find_one(EmailVerificationToken)
...
def test_hourly_email_limit(client, db_factory):
    ...
    for _ in range(5):
        ...
        with db_factory() as db:
            for item in db.find(PasswordResetToken):
                item.created_at = datetime.now(UTC) - timedelta(minutes=2)
                db.save(item)
    limited = client.post("/api/v1/auth/forgot-password", json={"email": "delivery@example.com"})
...
def test_production_api_hides_tokens_and_queues_unknown_accounts_privately(
    ...
    with db_factory() as db:
        assert len(list(db.find(AccountEmail))) == 2
...
def test_smtp_uses_tls_and_public_link(monkeypatch):
    ...
    settings = Settings(
        ...
        public_app_url="https://fuuud.example",
    )
...
def test_mail_configuration_rejects_insecure_production_links():
    settings = Settings(
        ...
        jwt_secret="x" * 32,
        smtp_host="smtp.example.com",
        public_app_url="http://fuuud.example",
    )
...
def test_pending_account_is_limited_and_admin_provisioning_revokes_sessions(client, db_factory):
    ...
    login = client.post(
        "/api/v1/auth/login", json={"email": "delivery@example.com", "password": "securePass123"}
    ).json()
    with db_factory() as db:
        provision_admin(db, "delivery@example.com")
        assert db.find_one(User).role == UserRole.admin
    assert (
        client.get(
